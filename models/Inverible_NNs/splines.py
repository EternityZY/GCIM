...
def _select_bins(param, idx):
    """
    Performs gather to select the bin in the correct way on batched inputs

    """
    idx = idx.clamp(min=0, max=param.size(-1) - 1)

    if len(idx.shape) >= len(param.shape):
        param = param.reshape((1,) * (len(idx.shape) - len(param.shape)) + param.shape)
        param = param.expand(idx.shape[:-2] + (-1,) * 2)
    return param.gather(-1, idx).squeeze(-1)
...
def _monotonic_rational_spline(inputs,
    ...
    bin_idx = _searchsorted(cumheights + eps if inverse else cumwidths + eps, inputs).unsqueeze(-1)     # 根据每个knots的横坐标，将输入
    ...
    if lambdas is not None:
        ...
        wb = torch.sqrt(input_derivatives / input_derivatives_plus_one) * wa    # wk+1 = sqrt(dk/dk+1) *wk
        ...
        yc = ((1.0 - input_lambdas) * wa * ya + input_lambdas * wb * yb) / \
            ((1.0 - input_lambdas) * wa + input_lambdas * wb)                   #  ym
        if inverse:                                 #
            #
            ...
            theta = (inputs - input_cumwidths) / input_widths                   # φ = (x - xk)/(xk+1 - xk)
            ...
            # u_k = spline(input∈bin_k, [w_k,h_k,d_k,λ_k])
    ...
    # Apply the identity function outside the bounding box 在包围框外应用恒等函数
    outputs[outside_interval_mask] = inputs[outside_interval_mask]
    logabsdet[outside_interval_mask] = 0.0

    inf_mask = torch.isinf(outputs)
    nan_mask = torch.isnan(outputs)
    outputs[inf_mask] = inputs[inf_mask]
    logabsdet[inf_mask] = 0.0

    outputs[nan_mask] = inputs[nan_mask]
    logabsdet[nan_mask] = 0.0
